...
ffi.cdef([[  typedef void MagickWand;
  ...
  MagickBooleanType MagickGetImageLength(MagickWand*, MagickSizeType*);
...
-- copied from http://www.imagemagick.org/download/releases/ImageMagick-6.9.5-10.tar.xz/magick/resample.h
local resample_filters = [[ typedef enum {
                                UndefinedFilter,
                                PointFilter,
                                BoxFilter,
                                TriangleFilter,
...
-- copied from https://pkgs.alpinelinux.org/package/v3.4/main/x86_64/imagemagick
local lib = try_to_load("libMagickWand-6.Q16.so.2")
...
do
  local _base_0 = {
    ...
    get_length = function(self)
        local img_len = ffi.new("MagickSizeType[1]")
        lib.MagickGetImageLength(self.wand, img_len)
        return img_len[0]
    end,
...
local function smart_resize(img, size_str)
  if type(img) == "string" then
    img = assert(load_image(img))
  end
  local src_w, src_h = img:get_width(), img:get_height()
...
local function free_raw_blob(blob)
    lib.MagickRelinquishMemory(blob)
end
return {
