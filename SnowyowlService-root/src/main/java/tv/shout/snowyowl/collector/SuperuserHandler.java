...
implements WdsPublisher
{
    ...
    public CollectorEndpoint[] getCollectorEndpoints()
    {
        CollectorEndpointHandler[] handlers = new CollectorEndpointHandler[] {
                ...
                new CollectorEndpointHandler(new CollectorEndpoint("/snowladmin/mme/killprocess", ConnectionType.ANY))
                .withValidRoles(new HashSet<>(Arrays.asList(SUPERUSER)))
                .withMessageHandlerFunction(
                        (message, logMessageTag) ->
                        killMMEProcess(message.getProperties(), message.getMessageId())),
    ...
    private Map<String, Object> killMMEProcess(Map<String, String> props, String messageId)
    throws PublishResponseError
    {
        String processId = getParamFromProps(props, messageId, "killMMEProcess", "processId", true);

        _engineCoordinator.killMMEProcess(processId);
