...
//TODO: don't do this intermediate step. just generate the collapsed value
    ...
    * @param payoutModel
    * @param playerCount
    * @param ignorePlayerCountLargerThanMaxPlayerCount in all cases except one, it's bad for the player count to be larger than the max player count. this would
    *      mean that the algorithm scales up, which is illegal. HOWEVER, after a game is over and being scored, it is possible that the player count could legitimately
    *      be larger than the max player count, and thus this needs to be allowed for. How does this happen? If the player count is not a power of 2 (2/4/8/16/32/etc),
    *      extra players sometimes need to be added at the beginning of rounds to get an even number of players. Thus for a game with a max of 100 players, there can
    *      actually be 103 players total.
    * @return
    * @throws PayoutManagerException
    */
   ...
   throws PayoutManagerException
   {
     ...
     */
    @Override
    public List<PayoutModelRound> getAdjustedPayoutModelRounds(int numPlayers, PayoutModel payoutModel,
            GamePayout gamePayout) throws PayoutManagerException
    {
...
_logger.info(">>> netProfitPerPlayer (pre negative adjust): " + netProfitPerPlayer);
...
//_logger.info(">>> releasing all sponsors for game: " + game.getId());
    ...
    //TODO: move off main processing loop
...
_logger.info("affiliatePayout::player " + s.getNickname() + " has no affiliate plan. skipping");
                continue;
            }
