...
public class BaseAiEngine
{
    ...
    public void submitAnswer(final AiQuestionBundle bundle)
    {
        ...
        //FUTURE: this isn't necessary if we're using doctoredTime (which is most likely the case in production environments)
        new Thread() {
            @Override
            public void run()
            {
...
_logger.info("AI " + bot.getSubscriberId() + " is submitting answer " + answerId + " on sqa " + bundle.sqaId + ". time: " + answerDurationMs);
                    _commonBusinessLogic.submitAnswer(bot, bundle.sqaId, answerId, answerDurationMs);
