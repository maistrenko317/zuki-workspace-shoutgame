...
//    }
    ...
    public void printPayouts(int payoutModelId, int numPlayers) throws PayoutManagerException
    {
        float minPayoutAmount = 1F;
        ...
        System.out.println(MessageFormat.format(
            "PayoutModel: {0}, entranceFee: ${1}, minPayoutAmount: ${2}, #players: {3}, scalePyouts: {4}",
            pm.getName(), pm.getEntranceFeeAmount(), minPayoutAmount, numPlayers, pm.isScalePayout()));
        ...
        if (income == 0) {
            if (payouts.get(0) > 0) {
                payoutPercentage = 100.0D;
            } else {
                payoutPercentage = 0.0D;
            }
        } else {
            payoutPercentage = (payouts.get(0)/income)*100;
        }

        System.out.println(MessageFormat.format(
            "income: ${0}, payout: ${1}, payout: {2}%, netProfitPerPlayer: ${3}",
            income, payouts.get(0), payoutPercentage, netProfitPerPlayer));
    }

    public static void main(String[] args) throws PayoutManagerException
    {
        PayoutManagerTester tester = new PayoutManagerTester();
        int payoutManagerId = 20;

        //tester.printPayouts(payoutManagerId, 1_048_576);
        //tester.printPayouts(payoutManagerId, 524_288);
        //tester.printPayouts(payoutManagerId, 131_072);
        //tester.printPayouts(payoutManagerId, 100_000);
        //tester.printPayouts(payoutManagerId, 16_384);
        //tester.printPayouts(payoutManagerId, 10_486);
        //tester.printPayouts(payoutManagerId, 10_000);
        //tester.printPayouts(payoutManagerId, 4096);
        //tester.printPayouts(payoutManagerId, 1024);
        //tester.printPayouts(payoutManagerId, 256);
        tester.printPayouts(payoutManagerId, 64);
        //tester.printPayouts(payoutManagerId, 30);
        //tester.printPayouts(payoutManagerId, 10);
        //tester.printPayouts(payoutManagerId, 2);
    }
}
