    ...
    public CollectorEndpoint[] getCollectorEndpoints()
    {
        CollectorEndpointHandler[] handlers = new CollectorEndpointHandler[] {
            ...
            new CollectorEndpointHandler(new CollectorEndpoint("/snowl/game/extralives/purchase", ConnectionType.ANY))
                ...
                new CollectorEndpointHandler(new CollectorEndpoint("/snowl/game/extralives/query", ConnectionType.ANY))
                .withValidRoles(new HashSet<>(Arrays.asList()))
                .withMessageHandlerFunction(
                    (message, logMessageTag) ->
                    queryExtraLives(message.getProperties(), message.getMessageId())),
...
//                    cancelPoolPlay(message.getProperties(), message.getMessageId())),
    ...
    throws PublishResponseError
    {
        ...
        existingGamePlayerRecord.setTotalLives(existingGamePlayerRecord.getTotalLives() + numLivesToPurchase);
        _shoutContestService.updateGamePlayer(existingGamePlayerRecord);

        return null;
    }

    private Map<String, Object> queryExtraLives(Map<String, String> props, String messageId)
    throws PublishResponseError
    {
        String docType = "queryExtraLives";
        ...
        return new FastMap<>(
                "gameId", game.getId(),
                "totalLives", existingGamePlayerRecord.getTotalLives(),
                "remainingLives", existingGamePlayerRecord.getCountdownToElimination()
        );
    }
...
//    }
    ...
    throws PublishResponseError
    {
        ...
        //store it
        _gameStatsHandler.setGameStats(new GameStats(gameId).withTwitchConsoleFollowedSubscriberId(twitchConsoleFollowedSubscriberId));
    ...
    private Map<String, Object> getQuestionDecryptKeyTransaction(Object param)
    {
        ...
        GameStats gameStats = _gameStatsHandler.getGameStats(sqa.getGameId());
        if (gameStats != null) {
        ...
        //send delayed message bus message for ANSWER_TIMEOUT_AT
        Round round = _shoutContestService.getRound(sqa.getRoundId());
        Game game = _shoutContestService.getGameNoFat(round.getGameId());
        DelayedMessage delayedMessage = HandlerAnswer.getAnswerTimeoutMessage(round.getPlayerMaximumDurationSeconds()*1_000L, subscriberQuestionAnswerId, game.getEngineType());
        if (_busLogger.isDebugEnabled()) {
            _busLogger.debug(MessageFormat.format("GameHandler sending delayed message: {0}", delayedMessage.type));
        }
    ...
    private Map<String, Object> submitAnswer(Map<String, String> xprops, String xmessageId)
    throws PublishResponseError
    {
        Subscriber xsubscriber = getSubscriber(xprops, xmessageId, "submitAnswer");

        wrapInTransaction((params) -> {
            @SuppressWarnings("unchecked")
            Map<String, String> props = (Map<String, String>) ((Object[])params)[0];
            String messageId = (String) ((Object[])params)[1];
            Subscriber subscriber = (Subscriber) ((Object[])params)[2];

            String sqaId = props.get("subscriberQuestionAnswerId");
            String selectedAnswerId = props.get("selectedAnswerId");

            SubscriberQuestionAnswer sqa = _commonBusinessLogic.getSubscriberQuestionAnswerWithPrechecks(props.get(PARM_TO_WDS), messageId, "submitAnswer", subscriber, sqaId, selectedAnswerId);
            Game game = _shoutContestService.getGameNoFat(sqa.getGameId());

            Message msg = HandlerAnswer.getAnswerMessage(sqa, selectedAnswerId, game.getEngineType());
            if (_busLogger.isDebugEnabled()) {
                _busLogger.debug("GameHandler sending message: " + msg.type);
            }
            _messageBus.sendMessage(msg);

            return null;
        }, new Object[] {xprops, xmessageId, xsubscriber});
