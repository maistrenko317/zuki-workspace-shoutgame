...
IAutodiscoverRedirectionUrl, IFunctionDelegate {
	...
	URISyntaxException {
		...
		request.setPreferredWebAuthMethod(getPreferredWebAuthMethod());
		try {
		...
		for (int currentHop = 0; currentHop < AutodiscoverMaxRedirections; currentHop++) {
			...
			if (request != null) {
				...
				if (this.tryGetRedirectionResponse(request, outParam)) {
                    ...
                    this.traceMessage(TraceFlags.AutodiscoverResponseCode, Integer.toString(request.getResponseCode()));
                    return false;
				} else {
	 ...
	 */
	public void setDnsServerAddress(String value) {
