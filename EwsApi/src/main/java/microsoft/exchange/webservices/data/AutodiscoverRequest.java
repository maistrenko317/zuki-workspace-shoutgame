...
abstract class AutodiscoverRequest {
	...
	protected AutodiscoverResponse internalExecute()
		...
		try {
			...
			if (request.getResponseCode() == 401) {
                AutodiscoverResponse response = this
                        ...
                        response.setErrorCode(AutodiscoverErrorCode.InvalidUser);
                        return response;
			} else if (AutodiscoverRequest.isRedirectionResponse(request)) {
				AutodiscoverResponse response = this
		...
		} catch (IOException ex)
        {
            ...
            // Wrap exception
            throw new ServiceRequestException(ex.getMessage(), ex);
        } catch (Exception ex) {
