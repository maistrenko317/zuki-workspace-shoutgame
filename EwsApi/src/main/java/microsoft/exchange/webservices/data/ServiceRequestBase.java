...
abstract class ServiceRequestBase {
	...
	protected void writeToXml(EwsServiceXmlWriter writer) throws Exception {
		...
		boolean emitTzHeader = false;
        if (this instanceof CreateRequest<?, ?>) {
            CreateRequest<?, ?> cr = (CreateRequest<?,?>)this;
            Iterable<?> itb = cr.getObjects();
            Iterator<?> it = itb.iterator();
            while (it.hasNext()) {
                Object o = it.next();
                if (o instanceof ServiceObject) {
                    ServiceObject so = (ServiceObject) o;
                    if (so.getIsTimeZoneHeaderRequired(false)) {
                        emitTzHeader = true;
                        break;
                    }
                }
            }
        } else if (this instanceof UpdateItemRequest) {
            UpdateItemRequest cr = (UpdateItemRequest)this;
            Iterable<?> itb = cr.getItems();
            Iterator<?> it = itb.iterator();
            while (it.hasNext()) {
                Object o = it.next();
                if (o instanceof ServiceObject) {
        ...
        if (
            (this.getService().getRequestedServerVersion().ordinal() == ExchangeVersion.Exchange2007_SP1.ordinal() || emitTzHeader) && 
            (!this.getService().getExchange2007CompatibilityMode())
           ) {
            TimeZoneDefinition tzd = this.getService().getTimezoneDefinition();
            if (tzd != null) {
                writer.writeStartElement(XmlNamespace.Types, XmlElementNames.TimeZoneContext);
                tzd.writeToXml(writer);
                writer.writeEndElement(); // TimeZoneContext
                writer.setTimeZoneHeaderEmitted(true);
            }
        }
	...
	throws ServiceLocalException, Exception
	{
		...
		{
		    String message = e == null ? "exception is null" : e.getMessage() == null ? "exception message is null" : e.getMessage();
	        throw new ServiceRequestException(message, e);
		    
			// Wrap exception.
//			throw new ServiceRequestException(String.format(Strings.
//					ServiceRequestFailed, e == null ? "null" : e.getMessage()), e);
		}
